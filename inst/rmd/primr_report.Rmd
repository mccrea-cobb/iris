...
source("../functions/query_iris.R")
source('../functions/primr_get_iris.R')
read_chunk('../plots.R')
```

```{r get_data, echo=FALSE}

dim_survey <- query_iris(con, "Refuges", "DimSurvey") %>%
  filter(Station %like% "FF07%") %>%
  #filter(SurveyStatus != "Historic") %>%
  collect()

dim_annual_activity <- query_iris(con, "Refuges", "DimSurveyAnnualActivity")
dim_annual_activity_ak <- dim_annual_activity %>%
  filter(str_detect(Station, "FF07")) %>%
  collect()
